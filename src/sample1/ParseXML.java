                ...
                for (int j = 0; j < vMergeIntoXMLDocumentChildNodes.getLength(); j++) { // looping MDM
                    ...
                    if (elementsMatchOnBusinessKeys(vXMLDocumentToMergeChildNodes.item(i), vMergeIntoXMLDocumentChildNodes.item(j))) {
                        ...
                        if (vXMLDocumentToMergeChildNodes.item(i).getNodeName().equalsIgnoreCase("BaseInvestmentProduct")) {
                            ...
                            loopBaseInvetmentProductElement(vMergeIntoXMLDocumentChildNodes, keyValues);
...
//				}
    ...
    private static void addSensitiveFieldsToRequest(Document vXMLDocumentToMerge, HashMap<String, String> keyValues) {
        NodeList nodeInterestRateExpiryDate = vXMLDocumentToMerge.getElementsByTagName("interestRateExpiryDate");
        ...
        for (Map.Entry<String,String> entry : keyValues.entrySet()) {
            ...
            nodeInterestRateExpiryDate.item(0).getParentNode().insertBefore(xmlElement, nodeInterestRateExpiryDate.item(0));
        }
    }

    private static void loopBaseInvetmentProductElement(NodeList nodeList, HashMap<String, String> hm) {
        for (int count = 0; count < nodeList.getLength(); count++) {
            Node tempNode = nodeList.item(count);

            // make sure it's element node. Loop through to retrieve sensitive fields from MDM Arrangment XValueXML
            if (tempNode.getNodeType() == Node.ELEMENT_NODE) {
                ...
                // Sensitive Field - duration
                if (tempNode.getNodeName().equalsIgnoreCase("duration")) {
                    hm.put(tempNode.getNodeName(), tempNode.getTextContent());
                }

                if (tempNode.hasChildNodes()) {
                    // loop again if has child nodes
                    loopBaseInvetmentProductElement(tempNode.getChildNodes(), hm);
